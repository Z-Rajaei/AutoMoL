...
class AutoModelLearnerBase:
    ...
    def _finish_fit(self, dataset=None):
        ...
        """
        try:
            kwargs = {"dataset": dataset, "metric": self.metric}
            # autogl's own InMemoryDataset lacks .data.y, so pass the test labels
            # explicitly whenever the dataset exposes a test_index.
            if dataset is not None and hasattr(dataset, "test_index"):
                kwargs["label"] = torch.cat(
                    [dataset[i].y for i in dataset.test_index]
                ).view(-1)
            value = self.solver.evaluate(**kwargs)

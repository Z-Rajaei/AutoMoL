...
class ModelDataset(Dataset):
    ...
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None, mapping_file=None, pyg_subdir="pyg_torch_data"):
        """
        ...
        pyg_subdir : str, optional
            Subdirectory of ``root`` containing the transformed_*.pt graphs,
            by default 'pyg_torch_data'.
        """
        self.root = root
        self.pyg_dir = os.path.join(root, pyg_subdir)
    ...
    def _create_class_map(self):
        ...
        """
        # Prefer the cluster mapping file (graph id -> cluster label) when present;
        # it is the authoritative label source for this dataset.
        if self.mapping_file and os.path.exists(self.mapping_file):
            with open(self.mapping_file, 'r', encoding='utf-8') as f:
                raw = json.load(f)
            clusters = sorted(set(raw.values()))
            remap = {c: i for i, c in enumerate(clusters)}
            class_map = {graph_id: remap[c] for graph_id, c in raw.items()}
            logger.info(
                f"Loaded {len(class_map)} labels for {len(remap)} classes from {self.mapping_file}"
            )
            return class_map

        # Fallback: extract model names from file paths
        model_names = set()
    ...
    @property
    def data(self):
        """AutoGL compatibility: expose all graph labels via a Data object's .y."""
        if self._label_mapping is None:
            labels = torch.cat([self[i].y for i in range(len(self))]).view(-1)
            self._label_mapping = Data(y=labels)
        return self._label_mapping

    def _split_subset(self, name):
    ...
    def train_split(self):
        """AutoGL compatibility: subset built from the train_index set by graph_random_splits."""
        return self._split_subset("train_index")

    @property
    def val_split(self):
        """AutoGL compatibility: subset built from the val_index set by graph_random_splits."""
        return self._split_subset("val_index")

    @property
    def test_split(self):
        """AutoGL compatibility: subset built from the test_index set by graph_random_splits."""
        return self._split_subset("test_index")

...
def main():
    ...
    print("attribute coverage (declared feature -> nodes carrying it):")
    for s in enc.feature_specs:
        print(f"  {s['feature']:30s} {s.get('encoding'):10s} "
              f"{enc.attribute_coverage.get(s['feature'], 0)}")
